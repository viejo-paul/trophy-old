      ...
      <div className={`${baseClass} ${isDark ? darkClass : lightClass} relative`}>
        {[0,1,2,3,4,5,6,7,8].map(i => (
          <div key={i} className="w-1.5 h-1.5 flex items-center justify-center">
            {dotMap[value]?.includes(i) && <div className={`w-1.5 h-1.5 rounded-full ${dotColor}`} />}
          </div>
        ))}
      </div>
    );
      ...
      return (
        <div className={`bg-black/40 border ${borderColor} rounded p-2 mb-2`}>
          ...
          <div className="flex flex-col items-center mb-2">
             ...
             {msg.lightRolls && msg.lightRolls.length > 0 && (
               <div className="flex items-center gap-2 mb-1">
                 ...
                 {msg.lightRolls.map((r, i) => <D6 key={`l-${i}`} value={r} isDark={false} />)}
               </div>
             )}
             {/* Dados Oscuros */}
             {msg.darkRolls && msg.darkRolls.length > 0 && (
               <div className="flex items-center gap-2">
                 ...
                 <div className="flex flex-wrap gap-1">
                    {msg.darkRolls.map((r, i) => <D6 key={`d-${i}`} value={r} isDark={true} />)}
                 </div>
               </div>
             )}
          </div>
          ...
          <div className="text-xs text-center px-1">
             ...
             {msg.outcomeDesc && <div className="text-[10px] text-gray-400 italic leading-tight">{msg.outcomeDesc}</div>}
